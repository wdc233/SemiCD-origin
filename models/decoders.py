...
def upsample(in_channels, out_channels, upscale, kernel_size=3):  # 512, 2, 8
    layers = []
    ...
    nn.init.kaiming_normal_(diff_conv1x1.weight.data, nonlinearity='relu')  # 使用正态分布对输入张量进行赋值
    ...
    up = nn.Upsample(scale_factor=upscale, mode='bilinear')  # 8
    layers.append(up)
    ...
    return nn.Sequential(*layers)  # 包含一个卷积层、relu激活曾、上采样层、分类卷积层


class MainDecoder(nn.Module):
    def __init__(self, upscale, conv_in_ch, num_classes):  # upscale = 8, decoder_in_ch = 512, num_classes = 2
...
class FeatureNoiseDecoder(nn.Module):
    def __init__(self, upscale, conv_in_ch, num_classes, uniform_range=0.3):
        ...
        self.upsample = upsample(conv_in_ch, num_classes, upscale=upscale)
        self.uni_dist = Uniform(-uniform_range, uniform_range)  # 方法将随机生成下一个实数，它在 [-0.3, 0.3] 范围内。

    def feature_based_noise(self, x):
        noise_vector = self.uni_dist.sample(x.shape[1:]).to(x.device).unsqueeze(0)  #  生成 1 x h x w的
        x_noise = x.mul(noise_vector) + x  # 给特征加噪声
        return x_noise
...
class VATDecoder(nn.Module):  # 8, 512, 2, 1e-6, 2.0, 2
...
def guided_cutout(output, upscale, resize, erase=0.4, use_dropout=False):  # None, 8, 0.4, (c, w)
    ...
    for mask in masks:
        ...
        try: # Version 3.x
            _, contours, _ = cv2.findContours(mask_np, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)  # 寻找轮廓
        except: # Version 4.x
...
class CutOutDecoder(nn.Module):
    ...
    def forward(self, x, pred=None, pertub=True):
        if pertub:
            maskcut = guided_cutout(pred, upscale=self.upscale, erase=self.erase, resize=(x.size(2), x.size(3)))   # None, 8, 0.4, c, w
            x = x * maskcut  #  点乘，maskcut为0的像素点值为0，相当于cut掉了
            x = self.upsample(x)
        else:
            x = self.upsample(x)
        return x
...
# 去掉背景，获取到变化区域mask
def guided_masking(x, output, upscale, resize, return_msk_context=True):  # x, pred, (h,w), 8
    if len(output.shape) == 3:
        masks_context = (output > 0).float().unsqueeze(1)  # 加了一个维度,全部变为0/1
    else:
        masks_context = (output.argmax(1) > 0).float().unsqueeze(1)

    masks_context = F.interpolate(masks_context, size=resize, mode='nearest')  # 上/下采样
...
class ObjectMaskingDecoder(nn.Module):
    ...
    # x_ul, output_ul.detach(), pertub = True
    def forward(self, x, pred=None, pertub=True):
        if pertub:
